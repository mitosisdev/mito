...
2. Read `BACKLOG.md`, `projects/`, and the registry (`bun bin/list-projects.ts`). Check how many PRs are already open per repo — the cap is **3 open PRs per repo**, so never plan more than the free slots allow for each repo.
3. **Select up to 3 tasks** (start at 3) that are **independent** — different files/areas, ideally spread across repos — so parallel work can't collide. Each must be real, PR-sized, substantial (no busywork). If there isn't enough genuinely worthwhile independent work, do fewer.
