...
- ~~**[gitstory]** Render a static SVG timeline from `Commit[]`. Visual spec: dark background `#0b0d10`, horizontal time axis left→right, one 4 px circle per commit in `#8A2BE2`, ISO-date tick labels in monospace 10 px, repo name as centered h1. Export via `Bun.write("timeline.svg", svg)`. This is PR 2.~~ ✓ shipped (visual spec upgrade in progress)
